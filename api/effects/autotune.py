...
class AutoTuneVoiceEffect(BaseVoiceEffect):
    ...
    def aclosest_pitch_from_scale(f0, scale):
        ...
        for i in np.arange(f0.shape[0]):
            sanitized_pitch[i] = f0.closest_pitch_from_scale(f0[i], scale)
    ...
    def process_audio(self, data):
        ...
        correction_function = self.closest_pitch if args.correction_method == 'closest' else \
            partial(self.aclosest_pitch_from_scale, scale=args.scale)

        # Perform the auto-tuning.
        pitch_corrected_y = self.autotune(y, sr, correction_function, args.plot)
    ...
    def start(self):
        ...
        super().start()
        print("Effet Auto-thune démarré. Prêt à moduler le son.")

    def stop(self):
        super().stop()
        print("Effet Auto-thune arrêté.")